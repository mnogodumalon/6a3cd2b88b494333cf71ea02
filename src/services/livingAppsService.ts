...
import type { Mitarbeitende, ItAusstattungZugaenge, OnboardingMassnahmen, OnboardingCheckliste, CreateMitarbeitende, CreateItAusstattungZugaenge, CreateOnboardingMassnahmen, CreateOnboardingCheckliste } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getMitarbeitende(): Promise<Mitarbeitende[]> {
    ...
    return enrichLookupFields(records, 'mitarbeitende');
  }
  static async getMitarbeitendeEntry(id: string): Promise<Mitarbeitende | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Mitarbeitende;
    return enrichLookupFields([record], 'mitarbeitende')[0];
  }
  static async createMitarbeitendeEntry(fields: CreateMitarbeitende) {
    return callApi('POST', `/apps/${APP_IDS.MITARBEITENDE}/records`, { fields: cleanFieldsForApi(fields as any, 'mitarbeitende') });
  }
  static async updateMitarbeitendeEntry(id: string, fields: Partial<CreateMitarbeitende>) {
    return callApi('PATCH', `/apps/${APP_IDS.MITARBEITENDE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'mitarbeitende') });
  }
  ...
  static async getItAusstattungZugaenge(): Promise<ItAusstattungZugaenge[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.IT_AUSSTATTUNG_ZUGAENGE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as ItAusstattungZugaenge[];
    return enrichLookupFields(records, 'it_ausstattung_&_zugaenge');
  }
  static async getItAusstattungZugaengeEntry(id: string): Promise<ItAusstattungZugaenge | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.IT_AUSSTATTUNG_ZUGAENGE}/records/${id}`);
    const record = { record_id: data.id, ...data } as ItAusstattungZugaenge;
    return enrichLookupFields([record], 'it_ausstattung_&_zugaenge')[0];
  }
  static async createItAusstattungZugaengeEntry(fields: CreateItAusstattungZugaenge) {
    return callApi('POST', `/apps/${APP_IDS.IT_AUSSTATTUNG_ZUGAENGE}/records`, { fields: cleanFieldsForApi(fields as any, 'it_ausstattung_&_zugaenge') });
  }
  static async updateItAusstattungZugaengeEntry(id: string, fields: Partial<CreateItAusstattungZugaenge>) {
    return callApi('PATCH', `/apps/${APP_IDS.IT_AUSSTATTUNG_ZUGAENGE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'it_ausstattung_&_zugaenge') });
  }
  static async deleteItAusstattungZugaengeEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.IT_AUSSTATTUNG_ZUGAENGE}/records/${id}`);
  }

  // --- ONBOARDING_MASSNAHMEN ---
  static async getOnboardingMassnahmen(): Promise<OnboardingMassnahmen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.ONBOARDING_MASSNAHMEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as OnboardingMassnahmen[];
    return enrichLookupFields(records, 'onboarding_maßnahmen');
  }
  static async getOnboardingMassnahmenEntry(id: string): Promise<OnboardingMassnahmen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.ONBOARDING_MASSNAHMEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as OnboardingMassnahmen;
    return enrichLookupFields([record], 'onboarding_maßnahmen')[0];
  }
  static async createOnboardingMassnahmenEntry(fields: CreateOnboardingMassnahmen) {
    return callApi('POST', `/apps/${APP_IDS.ONBOARDING_MASSNAHMEN}/records`, { fields: cleanFieldsForApi(fields as any, 'onboarding_maßnahmen') });
  }
  static async updateOnboardingMassnahmenEntry(id: string, fields: Partial<CreateOnboardingMassnahmen>) {
    return callApi('PATCH', `/apps/${APP_IDS.ONBOARDING_MASSNAHMEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'onboarding_maßnahmen') });
  }
  static async deleteOnboardingMassnahmenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.ONBOARDING_MASSNAHMEN}/records/${id}`);
  }

  // --- ONBOARDING_CHECKLISTE ---
  static async getOnboardingCheckliste(): Promise<OnboardingCheckliste[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.ONBOARDING_CHECKLISTE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as OnboardingCheckliste[];
    return enrichLookupFields(records, 'onboarding_checkliste');
  }
  static async getOnboardingChecklisteEntry(id: string): Promise<OnboardingCheckliste | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.ONBOARDING_CHECKLISTE}/records/${id}`);
    const record = { record_id: data.id, ...data } as OnboardingCheckliste;
    return enrichLookupFields([record], 'onboarding_checkliste')[0];
  }
  static async createOnboardingChecklisteEntry(fields: CreateOnboardingCheckliste) {
    return callApi('POST', `/apps/${APP_IDS.ONBOARDING_CHECKLISTE}/records`, { fields: cleanFieldsForApi(fields as any, 'onboarding_checkliste') });
  }
  static async updateOnboardingChecklisteEntry(id: string, fields: Partial<CreateOnboardingCheckliste>) {
    return callApi('PATCH', `/apps/${APP_IDS.ONBOARDING_CHECKLISTE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'onboarding_checkliste') });
  }
  static async deleteOnboardingChecklisteEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.ONBOARDING_CHECKLISTE}/records/${id}`);
  }
