...
interface MitarbeitendeDialogProps {
  ...
  defaultValues?: Mitarbeitende['fields'];
...
export function MitarbeitendeDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: MitarbeitendeDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<Mitarbeitende['fields']>);
      setPreview(null);
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "vorname": string | null, // Vorname\n  "nachname": string | null, // Nachname\n  "email_geschaeftlich": string | null, // Geschäftliche E-Mail-Adresse\n  "telefon": string | null, // Telefonnummer\n  "eintrittsdatum": string | null, // YYYY-MM-DD\n  "abteilung": LookupValue | null, // Abteilung (select one key: "entwicklung" | "design" | "projektmanagement" | "vertrieb" | "marketing" | "hr_personal" | "finanzen_controlling" | "it_infrastruktur" | "geschaeftsfuehrung" | "sonstiges") mapping: entwicklung=Entwicklung, design=Design, projektmanagement=Projektmanagement, vertrieb=Vertrieb, marketing=Marketing, hr_personal=HR & Personal, finanzen_controlling=Finanzen & Controlling, it_infrastruktur=IT & Infrastruktur, geschaeftsfuehrung=Geschäftsführung, sonstiges=Sonstiges\n  "position": string | null, // Position / Rolle\n  "vorgesetzte_vorname": string | null, // Vorname der/des Vorgesetzten\n  "vorgesetzte_nachname": string | null, // Nachname der/des Vorgesetzten\n  "arbeitsort": LookupValue | null, // Arbeitsort (select one key: "buero" | "remote" | "hybrid") mapping: buero=Büro, remote=Remote, hybrid=Hybrid\n  "vertragsart": LookupValue | null, // Vertragsart (select one key: "vollzeit" | "teilzeit" | "werkstudent" | "praktikum" | "ausbildung" | "freiberuflich") mapping: vollzeit=Vollzeit, teilzeit=Teilzeit, werkstudent=Werkstudent/in, praktikum=Praktikum, ausbildung=Ausbildung, freiberuflich=Freiberuflich\n}`;
  ...
  const DIALOG_INTENT = defaultValues ? 'Mitarbeitende bearbeiten' : 'Mitarbeitende hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'email_geschaeftlich': (
      <div key="email_geschaeftlich" className="space-y-1.5">
    ...
    'eintrittsdatum': (
      <div key="eintrittsdatum" className="space-y-1.5">
        ...
        <DatePicker
          id="eintrittsdatum"
          placeholder="Wann startet die Person?"
          mode="date"
    ...
    'abteilung': (
      <div key="abteilung" className="space-y-1.5">
        ...
        >
          <SelectTrigger id="abteilung"><SelectValue placeholder="Wähle eine Abteilung" /></SelectTrigger>
          <SelectContent>
    ...
    'arbeitsort': (
      <div key="arbeitsort" className="space-y-1.5">
    ...
    'vertragsart': (
      <div key="vertragsart" className="space-y-1.5">
        ...
        >
          <SelectTrigger id="vertragsart"><SelectValue placeholder="z. B. Vollzeit" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="none">—</SelectItem>
