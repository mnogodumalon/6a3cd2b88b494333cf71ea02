...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/OnboardingMassnahmen';
...
interface OnboardingMassnahmenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: OnboardingMassnahmen['fields']) => Promise<void>;
  defaultValues?: OnboardingMassnahmen['fields'];
...
export function OnboardingMassnahmenDialog({ open, onClose, onSubmit, defaultValues, recordId, mitarbeitendeList, enablePhotoScan = true, enablePhotoLocation = true }: OnboardingMassnahmenDialogProps) {
  const [fields, setFields] = useState<Partial<OnboardingMassnahmen['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<OnboardingMassnahmen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'onboarding_maßnahmen');
      await onSubmit(clean as OnboardingMassnahmen['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "mitarbeitender": string | null, // Display name from Mitarbeitende (see <available-records>)\n  "bezeichnung": string | null, // Bezeichnung der Maßnahme\n  "typ": LookupValue | null, // Typ der Maßnahme (select one key: "schulung" | "meeting" | "einfuehrungsgespraech" | "teamvorstellung" | "sonstiges_massnahme") mapping: schulung=Schulung, meeting=Meeting, einfuehrungsgespraech=Einführungsgespräch, teamvorstellung=Teamvorstellung, sonstiges_massnahme=Sonstiges\n  "datum_uhrzeit": string | null, // YYYY-MM-DDTHH:MM\n  "verantwortliche_vorname": string | null, // Vorname der verantwortlichen Person\n  "verantwortliche_nachname": string | null, // Nachname der verantwortlichen Person\n  "status_massnahme": LookupValue | null, // Status (select one key: "geplant" | "durchgefuehrt" | "abgesagt") mapping: geplant=Geplant, durchgefuehrt=Durchgeführt, abgesagt=Abgesagt\n  "notizen_massnahme": string | null, // Notizen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<OnboardingMassnahmen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Onboarding-Maßnahmen bearbeiten' : 'Onboarding-Maßnahmen hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'bezeichnung': (
      <div key="bezeichnung" className="space-y-1.5">
        ...
        <Input
          id="bezeichnung"
          placeholder="z. B. Einführungsgespräch IT"
    ...
    'datum_uhrzeit': (
      <div key="datum_uhrzeit" className="space-y-1.5">
        ...
        <DatePicker
          id="datum_uhrzeit"
          placeholder="Wann ist die Maßnahme geplant?"
          mode="datetime"
    ...
    'verantwortliche_vorname': (
      <div key="verantwortliche_vorname" className="space-y-1.5">
    ...
    'status_massnahme': (
      <div key="status_massnahme" className="space-y-1.5">
    ...
    'notizen_massnahme': (
      <div key="notizen_massnahme" className="space-y-1.5">
        <Label htmlFor="notizen_massnahme">Notizen</Label>
        <Textarea
          id="notizen_massnahme"
          placeholder="Agenda, Ergebnisse, Besonderheiten..."
  ...
  const FIELD_LABELS: Record<string, string> = {"mitarbeitender": "Mitarbeitende/r", "bezeichnung": "Bezeichnung der Maßnahme", "typ": "Typ der Maßnahme", "datum_uhrzeit": "Datum und Uhrzeit", "verantwortliche_vorname": "Vorname der verantwortlichen Person", "verantwortliche_nachname": "Nachname der verantwortlichen Person", "status_massnahme": "Status", "notizen_massnahme": "Notizen"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.ONBOARDING_MASSNAHMEN} recordId={recordId} />
              </div>
            )}
          </div>
