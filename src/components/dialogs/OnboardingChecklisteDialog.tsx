...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/OnboardingCheckliste';
...
interface OnboardingChecklisteDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: OnboardingCheckliste['fields']) => Promise<void>;
  defaultValues?: OnboardingCheckliste['fields'];
...
export function OnboardingChecklisteDialog({ open, onClose, onSubmit, defaultValues, recordId, mitarbeitendeList, enablePhotoScan = true, enablePhotoLocation = true }: OnboardingChecklisteDialogProps) {
  const [fields, setFields] = useState<Partial<OnboardingCheckliste['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<OnboardingCheckliste['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'onboarding_checkliste');
      await onSubmit(clean as OnboardingCheckliste['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "mitarbeitender": string | null, // Display name from Mitarbeitende (see <available-records>)\n  "aufgabe_bezeichnung": string | null, // Aufgabenbezeichnung\n  "kategorie": LookupValue | null, // Kategorie (select one key: "hr" | "it" | "fuehrungskraft" | "team" | "sonstiges_kategorie") mapping: hr=HR & Personal, it=IT & Infrastruktur, fuehrungskraft=Führungskraft, team=Team, sonstiges_kategorie=Sonstiges\n  "faelligkeitsdatum": string | null, // YYYY-MM-DD\n  "verantwortliche_aufgabe_vorname": string | null, // Vorname der verantwortlichen Person\n  "verantwortliche_aufgabe_nachname": string | null, // Nachname der verantwortlichen Person\n  "erledigt": boolean | null, // Erledigt\n  "anmerkungen": string | null, // Anmerkungen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<OnboardingCheckliste['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Onboarding-Checkliste bearbeiten' : 'Onboarding-Checkliste hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'aufgabe_bezeichnung': (
      <div key="aufgabe_bezeichnung" className="space-y-1.5">
        <Label htmlFor="aufgabe_bezeichnung">Aufgabenbezeichnung</Label>
        <Input
          id="aufgabe_bezeichnung"
          placeholder="z. B. Zugänge IT-System"
    ...
    'faelligkeitsdatum': (
      <div key="faelligkeitsdatum" className="space-y-1.5">
        ...
        <DatePicker
          ...
          placeholder="Bis wann muss das erledigt sein?"
          mode="date"
    ...
    'verantwortliche_aufgabe_vorname': (
      <div key="verantwortliche_aufgabe_vorname" className="space-y-1.5">
    ...
    'verantwortliche_aufgabe_nachname': (
      <div key="verantwortliche_aufgabe_nachname" className="space-y-1.5">
    ...
    'anmerkungen': (
      <div key="anmerkungen" className="space-y-1.5">
        ...
        <Textarea
          id="anmerkungen"
          placeholder="Status, Fortschritt, Notizen..."
  ...
  const FIELD_LABELS: Record<string, string> = {"mitarbeitender": "Mitarbeitende/r", "aufgabe_bezeichnung": "Aufgabenbezeichnung", "kategorie": "Kategorie", "faelligkeitsdatum": "Fälligkeitsdatum", "verantwortliche_aufgabe_vorname": "Vorname der verantwortlichen Person", "verantwortliche_aufgabe_nachname": "Nachname der verantwortlichen Person", "erledigt": "Erledigt", "anmerkungen": "Anmerkungen"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.ONBOARDING_CHECKLISTE} recordId={recordId} />
              </div>
            )}
          </div>
